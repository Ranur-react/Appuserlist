...
import Kotak from '../../components/KotakMatakuliah';
...
export default class DosenPage extends Component {
  ...
  render() {
    const EachKotak = () => {
      if (!this.state.dataPthcer) {
        return <View style={styles.SubContainer}></View>;
      }
      return (
        <View style={styles.SubContainer}>
          {this.state.dataPthcer.map((value, i) => {
            return (
              <Kotak
                onClick={e =>
                  this.props.navigation.navigate('DetaiMatakuliah', {value})
                }
                data={value}
    ...
    return (
      <LinearGradient
        ...
        />
        <TouchableOpacity
          onPress={() => this.props.navigation.navigate('FormMatakuliahPage')}
